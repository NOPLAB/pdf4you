...
- 別途、翻訳中に「ローカル翻訳 → 外部サービス（OpenAI 互換 API）」へ切り替えるボタンを提示する（→ 第9節）。
...
# 外部翻訳サービス（OpenAI互換ならどこでも可）／ユーザー別APIキー
EXTERNAL_BASE_URL=https://openrouter.ai/api/v1   # 既定の切替先（/setkey でユーザー上書き可）
EXTERNAL_MODEL=openai/gpt-4o-mini     # 既定モデル（/setkey でユーザー上書き可）
...
## 9. ユーザー別APIキーと翻訳の外部切替（OpenAI 互換 API）

ローカル推論に加え、ユーザー自身の API キーで外部サービス（OpenAI 互換 API）を使って
翻訳できるようにする追加仕様。既定の切替先は `EXTERNAL_BASE_URL` / `EXTERNAL_MODEL`。

### 9.1 ユーザー別APIキー（DMスラッシュコマンド）
- `/setkey`: モーダルで API キー（と任意でモデル名・Base URL）を登録。空欄は既定値を使う。
  入力はチャットに残さず応答は ephemeral。
...
### 9.2 翻訳の外部切替（ボタン）
- 翻訳開始時に「外部サービスを使用」ボタンを提示（投稿者本人のみ操作可）。
- 押下時: 実行中のローカル翻訳を `cancel()` し、登録済みキーで外部サービス（`base_url`/`api_key`/`model`
